import time, string, requests, itertools, hashlib, json
...
class BSigner:
    ...
    def __srct_hash(self, data: str) -> str:
        _srct_hash = str(
            hashlib.md5(
                data.encode()
            ).hexdigest()
    ...
    def __base_string(self) -> str:
        base_str = self.__srct_hash(self.params)
        base_str = (
            base_str + self.__srct_hash(self.data) 
            if self.data
            else base_str + str("0" * 32)
        )
        base_str = (
            base_str + self.__srct_hash(self.cookies)
            if self.cookies
            else base_str + str("0" * 32)
        )
